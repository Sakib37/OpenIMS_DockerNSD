...
if [ -z "$name" ]; then
	# Actually this case should not happen, only if you renamed the config values ;)
	echo "$SERVICE : name not defined, will use default : scscf"
	name="scscf"
fi
...
python $SCRIPTS_PATH/$SUBSITUTE_SCRIPT $CFG_OUTPUT_FILE VAR_SCSCF_PORT%$port
...
python $SCRIPTS_PATH/$SUBSITUTE_SCRIPT $CFG_OUTPUT_FILE VAR_ICSCF_ENTRY%$icscf_entry
